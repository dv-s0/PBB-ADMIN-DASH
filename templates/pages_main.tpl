...
<h4 class="alert-heading text-center f16b">{$lang['Pages']}</h4>
</div>  
  <table class="table table-sm table-bordered table-striped  ml-1 f12r" style="width: 99%;">
  <thead>
    <tr class="f12r">
      <th scope="col">{$lang['Page_title']}</th>
...
{if {$PagesList['link']} !=''}
	<td><a target="_blank" href="{$PagesList['link']}">{$PagesList['title']}</a></td>
	<td><a target="_blank" href="{$PagesList['link']}">{$lang['View']}</a></td>

{else}
	<td><a target="_blank" href="../index.php?page=pages&amp;show=1&amp;id={$PagesList['id']}">{$PagesList['title']}</a></td>
	<td><a target="_blank" href="../index.php?page=pages&amp;show=1&amp;id={$PagesList['id']}">{$lang['View']}</a></td>
{/if}
	<td><a href="index.php?page=pages&amp;edit=1&amp;main=1&amp;id={$PagesList['id']}">{$lang['edit']}</a></td>
	<td><a href="index.php?page=pages&amp;del=1&amp;main=1&amp;id={$PagesList['id']}">{$lang['Delet']}</a></td>
</tr>
{else}
<tr valign="top" align="center">
<td  colspan="4">{$lang['No_Page']}</td>
</tr>
{/if}
{/Des::while}
</tbody>
 ...
 <div class="alert alert-success mx-1" role="alert">
<h4 class="alert-heading text-center f16b ">{$lang['External_Links_Pages']}</h4>
  </div>
...
<table class="table table-sm table-bordered table-striped  ml-1 f12r" style="width: 99%;">
  <thead>
    <tr class="f12r">
      ...
      <th scope="col">{$lang['Page_title']}</th>
	   <th scope="col">{$lang['Page_Link']}</th>
      </tr>
  </thead>
  <tbody>
...
{Des::while}{PagesList}
<tr valign="top" align="center">
<td>{$PagesList['sort']}</td>
  {if {$PagesList['link']} !=''}
	<td><a target="_blank" href="{$PagesList['link']}">{$PagesList['title']}</a></td>
	<td><input class="col-lg-8  form-control form-control-sm" type="text" value="{$PagesList['link']}" dir="ltr" size="81" /></td>
{else}
  	<td><a target="_blank" href="../index.php?page=pages&amp;show=1&amp;id={$PagesList['id']}">{$PagesList['title']}</a></td>
	<td><input class="col-lg-8  form-control form-control-sm" type="text" value="{$forum_adress}index.php?page=pages&amp;show=1&amp;id={$PagesList['id']}" dir="ltr" size="81" /></td>
	{/if}

</tr>
{if !{$PagesList['id']}}
<tr valign="top" align="center">
<td colspan="3">{$lang['No_links_to_external_pages']}</td>
</tr>
{/if}
{/Des::while}
</tbody>
